...
def issue_from_payload(payload: dict) -> ReferencedIssue:
...
def managed_label_names() -> set[str]:
    """Return labels owned by Nova issue metadata automation."""

    return {entry[0] for entry in MANAGED_KIND_LABELS.values()}


def copied_labels(issues: list[ReferencedIssue]) -> tuple[str, ...]:
    """Return managed issue labels to add to a pull request."""

    managed = managed_label_names()
    labels: list[str] = []
    seen: set[str] = set()
    for issue in issues:
        for label in issue.labels:
            if label not in managed or label in seen:
                continue
            labels.append(label)
            seen.add(label)
    return tuple(labels)
...
def shared_milestone(issues: list[ReferencedIssue]) -> tuple[str, int] | None:
    """Return a single shared source milestone, or None when absent/conflicting."""
...
def set_pull_request_milestone(
    client: GitHubClient,
    repository: str,
    pull_request: PullRequestIssue,
...
def referenced_issues(client: GitHubClient, repository: str, pull_request: PullRequestIssue) -> list[ReferencedIssue]:
    ...
    numbers = sorted(issue_reference_numbers(repository, pull_request.body))
    issues: list[ReferencedIssue] = []
...
def align_pull_request_metadata_command(args: argparse.Namespace) -> int:
    """Run pull request metadata alignment."""
...
def main() -> int:
    ...
    return align_pull_request_metadata_command(args)
